...
const registerUser = async (req, res) => {
  try {
    ...
    const activationToken = jwt.sign({ userId: user._id }, process.env.JWT_SECRET, {
    ...
    res.status(201).json({ message: 'User registered successfully. Check your email for activation instructions.' });
  } catch (error) {
...
const loginUser = async (req, res) => {
  try {
    ...
    if (!user.isActivated) {
      return res.status(401).json({ message: 'Authentication failed - account not activated' });
    }
...
const generatePasswordResetToken = async (req, res) => {
  try {
    ...
    res.status(200).json({ message: 'Password reset email sent. Check your inbox for instructions.' });
  } catch (error) {
    res.status(400).json({ error: error.message });
  }
};
...
export {registerUser, activateUser, loginUser, resetPassword, generatePasswordResetToken}
